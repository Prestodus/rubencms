...
class Default_Admin_Acl_Roles_Proxy {
	
	public function __construct() {
		$this->_mapper = new Default_Admin_Acl_Roles_Mapper();
	}
	...
	public function getAll() {
		...
		$allroles = $select->fetchAll();
		$allroles = $this->_mapper->objectify_multi($allroles);
		
		foreach ($allroles as $role) {
			$sql = "SELECT apr_permission_id FROM admin_permissions_relations WHERE apr_role_id = ?";
			$select = $db->prepare($sql);
			$select->execute(array($role->getId()));
			
			$permissions = $select->fetchAll();
			$new_permissions = array();
			foreach ($permissions as $permission) {
				$new_permissions[] = $permission['apr_permission_id'];
			}
			$role->setPermissions($new_permissions);
		}
